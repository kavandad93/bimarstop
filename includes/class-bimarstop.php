...
final class Plugin {
    ...
    public function register_roles(): void { self::register_bimarstop_roles(); self::create_chat_tables(); self::create_private_chat_tables(); }
    ...
    private function render_private_chat(int $partner_id, string $back_page): void {
        ...
        function sendMsg(){var v=input.value.trim();if(!v&&!file.files.length)return;var tempId="tmp-"+Date.now();var optimistic={id:tempId,message:v,sender:"شما",mine:true,time:"اکنون",attachment:null};if(file.files[0])optimistic.attachment={name:file.files[0].name,size:"در حال ارسال…",url:"#"};render(optimistic);box.scrollTop=box.scrollHeight;var f=new FormData();f.append("action","bimarstop_private_send_message");f.append("nonce","'.esc_js($nonce).'");f.append("thread_id","'.(int)$thread->id.'");f.append("message",v);if(file.files[0])f.append("chat_file",file.files[0]);send.disabled=true;input.value="";var selectedFile=file.files[0];clearFile();fetch("'.esc_url($ajax).'",{method:"POST",body:f}).then(r=>r.json()).then(function(x){send.disabled=false;if(!x.success){alert((x.data&&x.data.message)?x.data.message:"ارسال ناموفق بود.");load();return;}load();}).catch(function(){send.disabled=false;alert("ارتباط با سرور برقرار نشد.");});}
        send.onclick=function(e){e.preventDefault();sendMsg();};
